...
class ChipsInputState<T> extends State<ChipsInput<T>> implements TextInputClient {
  Set<T> _chips = <T>{};
  List<T?>? _suggestions;
  final StreamController<List<T?>?> suggestionsStreamController = StreamController<List<T>?>.broadcast();
  int _searchId = 0;
  TextEditingValue _value = const TextEditingValue();
  TextInputConnection? _textInputConnection;
  static SuggestionsBoxController? suggestionsBoxController;
  ...
  FocusNode get effectiveFocusNode => widget.focusNode ?? (focusNode ??= FocusNode());
  ...
  void initState() {
    ...
    _nodeAttachment = effectiveFocusNode.attach(context);
  ...
  void _handleFocusChanged() {
    if (effectiveFocusNode.hasFocus) {
      ...
      logs("_openInputConnection: ${effectiveFocusNode.hasFocus}");
      suggestionsBoxController?.open();
    } else {
      logs("_closeInputConnectionIfNeeded: ${effectiveFocusNode.hasFocus}");
      _closeInputConnectionIfNeeded();
      suggestionsBoxController?.close();
    }
    logs("Focus: ${effectiveFocusNode.hasFocus}");
    if (mounted) {
      logs("Focus: ${effectiveFocusNode.hasFocus}");
      setState(() {
  ...
  void _initOverlayEntry() {
    suggestionsBoxController?.overlayEntry = OverlayEntry(
      builder: (context) {
        final size = mounted ? renderBox?.size ?? Size.zero : Size.zero;
        final renderBoxOffset = mounted ? renderBox?.localToGlobal(Offset.zero) ?? Offset.zero : Offset.zero;
        ...
        const compositedTransformFollowerOffset = Offset.zero;
            ...
            if (snapshot.hasData && snapshot.data!.isNotEmpty) {
              final suggestionsListView = Material(
                ...
                child: ConstrainedBox(
                  ...
                  child: ListView.builder(
                    shrinkWrap: true,
                    padding: EdgeInsets.only(bottom: showTop ? 50 : 0),
              ...
              return Positioned(
                ...
                child: CompositedTransformFollower(
                    ...
                    child: FractionalTranslation(translation: const Offset(0, 0), child: suggestionsListView)
                    // !showTop
                    //     ? suggestionsListView
  ...
  void selectSuggestion(T data) {
    if (!_hasReachedMaxChips) {
      ...
      suggestionsStreamController.add(_suggestions);
          ...
          children: <Widget>[
            Flexible(
              ...
              child: Text(
                ...
                style: widget.textStyle ?? theme.textTheme.bodyMedium!.copyWith(height: 1.5),
              ),
            ),
            Flexible(
              flex: 0,
              child: TextCursor(resumed: effectiveFocusNode.hasFocus),
            ),
          ],
        ),
      ),
          ...
          children: <Widget>[
            GestureDetector(
              ...
              child: InputDecorator(
                ...
                baseStyle: const TextStyle(fontWeight: FontWeight.w400, fontSize: 12.5, color: AppColorConstants.appBlack),
                child: Wrap(
